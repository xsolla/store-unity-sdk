...
namespace Xsolla
{
	public class AndroidFilesProcessor : IPreprocessBuildWithReport, IPostprocessBuildWithReport
	{
		...
		private static void PatchManifest(string filePath)
		{
			...
			var attribute = providerNode.Attributes["android:name"];
			if (attribute == null)
				CreateXmlAttribute("android", "name", "androidx.startup.InitializationProvider", providerNode, root);

			attribute = providerNode.Attributes["android:authorities"];
			if (attribute == null)
				CreateXmlAttribute("android", "authorities", "${applicationId}.androidx-startup", providerNode, root);

			attribute = providerNode.Attributes["tools:node"];
			if (attribute == null)
				CreateXmlAttribute("tools", "node", "remove", providerNode, root);
		...
		private static void CreateXmlAttribute(string prefix, string name, string value, XmlElement node, XmlNode root)
		{
			var xmlNamespace = root.GetNamespaceOfPrefix(prefix);
