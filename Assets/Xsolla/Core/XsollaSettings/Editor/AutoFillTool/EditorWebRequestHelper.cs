...
namespace Xsolla.Core.AutoFillSettings
{
	public class EditorWebRequestHelper : MonoBehaviour
	{
		...
		{
			if (requestBody != null)
				WebRequestHelper.Instance.AttachBodyToRequest(request,requestBody);

			if (requestHeaders != null)
				WebRequestHelper.Instance.AttachHeadersToRequest(request,requestHeaders);
		...
		private static void OnEditorUpdate()
		{
			...
			while (index < _requestQueue.Count)
			{
				...
				{
					if (WebRequestHelper.CheckNoErrors(cur.request, ErrorCheckType.CommonErrors, out Error error))
