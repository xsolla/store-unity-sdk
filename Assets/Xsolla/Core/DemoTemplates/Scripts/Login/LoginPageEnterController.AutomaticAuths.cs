	...
	public partial class LoginPageEnterController : LoginPageController
	{
		...
		private void RunAutomaticAuths()
		{
			...
			Action<string> onSuccessfulAutomaticAuth = token =>
				DemoController.Instance.GetImplementation().ValidateToken(token, onSuccess: validToken => CompleteSuccessfulAuth(validToken), onError: _ => onFailedAutomaticAuth.Invoke(null));
